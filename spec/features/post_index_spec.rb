...
RSpec.describe 'PostIndex', type: :feature do
  before :each do
    @user_one = User.create(name: 'Selma', photo: 'https://avatars.githubusercontent.com/u/61165443?s=400&u=390894ffec3dbb185885f2366dfaff1309c5b355&v=4', bio: 'Full-Stack Developer from Namibia', posts_counter: 2)    
      
    @user_two = User.create(name: 'Ernest', photo: 'https://avatars.githubusercontent.com/u/92553079?v=4', bio: 'Web Developer from Kenya', posts_counter: 3)    
  ...
  describe 'Post index page features' do
    ...
    end    it "should show the user's profile picture" do
      expect(page).to have_css("img[src*='#{@user_one.photo}']")
    end    it "should show the user's name" do
      expect(page).to have_content(@user_one.name)
    end    it 'should show the number of posts of the user one' do
      expect(page).to have_content("Number of Posts: #{@user_one.posts_counter}")
    end    it "should show the user's first three posts" do
      @user_posts.each do |post|
        expect(page).to have_content(post.title)
        expect(page).to have_content(post.text)
      end
    end    it 'should show the first comment on a post' do
      expect(page).to have_content(@first_comment.text)
    end    it 'should show the number of comments on each post' do
      @user_posts.each do |post|
        expect(page).to have_content("Comments: #{post.comments_counter}")
      end
    end    it 'should show the number of likes each post has' do
      @user_posts.each do |post|
        expect(page).to have_content("Likes: #{post.likes_counter}")
      end
    end    it 'to show the Pagination buttons' do
        expect(find_link('Pagination')).to be_visible
      end    it 'should redirect to the post show page when a post is clicked' do
